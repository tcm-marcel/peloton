...
type::TypeId PostgresStringToTypeId(const std::string& str) {
  // TODO(marcel) fill in missing postgres type strings
  if (str == "bool") {
    ...
    throw ConversionException(StringUtil::Format(
        "No TypeId conversion from postgres string '%s'", str.c_str()));
  }
